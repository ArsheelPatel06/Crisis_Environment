...
_RED_TEAM_TO_WORLD: Dict[str, str] = {
...
class CyberCrisisEnv:
    ...
    red_team: RedTeamAgent = field(init=False)
    finance_stakeholder: FinanceStakeholder = field(init=False)
    ...
    def __post_init__(self) -> None:
        ...
        self.noop_streak = 0
        self._init_stakeholder_agents()

    def _init_stakeholder_agents(self) -> None:
        self.red_team = RedTeamAgent()
        self.finance_stakeholder = FinanceStakeholder()
    ...
    @property
    def infected_nodes(self) -> int:
        """Aggregate compromise signal (used by determinism tests)."""

        return sum(int(self.world.nodes[n].compromise_stage) for n in KILL_CHAIN)
    ...
    def _world_key_for_red_team_position(self, position: str) -> str:
        return _RED_TEAM_TO_WORLD.get(position, "api_gateway")

    def _network_state_for_agents(self, action: Action) -> NetworkState:
        """Map simulator world + defender action into Pydantic ``NetworkState`` for agents."""

        node_key = self._world_key_for_red_team_position(self.red_team.position)
        nst = self.world.nodes[node_key]
        defender_detected_red_team = nst.monitoring_level >= 2 or bool(nst.isolated)
        ...
        isolation_proposed = any(ns.isolated for ns in self.world.nodes.values())
        shutdown_requested = int(self.world.nodes["database"].compromise_stage) >= 2
        public_comms_pending = self.stakeholders.pending is not None
        nodes_under_stress = [n for n in KILL_CHAIN if int(self.world.nodes[n].compromise_stage) > 0]

        return NetworkState(
            timestep=self.step_count,
            threat_level=float(threat_level),
            status=status,
            isolation_proposed=isolation_proposed,
            shutdown_requested=shutdown_requested,
            public_comms_pending=public_comms_pending,
            nodes_under_stress=nodes_under_stress,
            summary="",
            defender_detected_red_team=defender_detected_red_team,
        )
    ...
    def _step_full_episode(self, action: Action, info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        network_state = self._network_state_for_agents(action)
        self.red_team.step(network_state)

        had_poison = {
            "Finance": self.finance_stakeholder.poison_payload is not None,
            "Engineering": self.engineering_stakeholder.poison_payload is not None,
            "PR": self.pr_stakeholder.poison_payload is not None,
        }

        self._last_stakeholder_messages = [
            self.finance_stakeholder.generate_message(network_state, self.step_count),
            self.engineering_stakeholder.generate_message(network_state, self.step_count),
    ...
    def _build_observation(self, *, task_score: float) -> Observation:
        ...
        return Observation(
            ...
            stakeholder_messages=list(self._last_stakeholder_messages),
            stakeholder_trust_scores=dict(self.stakeholder_trust_scores),
        )
